const colors = ['#008080', '#6A5ACD'];
        ...
        const play = ()=>{
            var interval = setInterval(() => {
                if (!lost) {
                    move();
                    draw();
                    if (((x+10>=bar.x && x+10<=bar.x+bar.width) || (x-10>=bar.x && x-10<=bar.x+bar.width)) && y+10===bar.y) {
                        stepY*=-1;
                        if(moveLeft && Math.sign(stepX)===1) stepX *= -1;
                        else if(moveRight && Math.sign(stepX)===-1) stepX *= -1; 
                        // index = Math.floor(Math.random() * 13);
                        // c.style.backgroundColor = colors[Math.floor(Math.random() * 13)];
                    }
                    if((x+10===bar.x || x-10===bar.x+bar.width) && y+10>bar.y) stepX*=-1;
                    
                    if (x+10>=c.width || x-10<=0) stepX*=-1; 
                    // if (y+10>=c.height || y-10<=0) stepY*=-1; 
                    if (y-10<=0) stepY*=-1;
        ...
        const move = () => {
            if (moveLeft) {
                if(bar.x>0) bar.x-=2;
                // if(x+10>bar.x && y>bar.y) x-=2;
            }
            if (moveRight) {
                if(bar.x+bar.width<c.width) bar.x+=2;
                // if(x-10<bar.x+bar.width && y>bar.y) x+=2;
            }
        }
        const shutdown = () => {
